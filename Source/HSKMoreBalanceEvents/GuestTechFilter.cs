...
namespace HSKMoreBalanceEvents
{
    ...
    // тоже (границы guestMaxTechAhead / guestMaxTechBehind в EventSettings.xml).
    ...
    public static class GuestTechFilter
    {
        ...
        static GuestTechFilter()
        {
            ...
            {
                Log.Warning("[HSKMoreBalanceEvents] GuestTechFilter: API Hospitality изменилось — фильтр гостей отключён.");
                return;
            }
            ...
            var harmony = new Harmony("linya.hskmorebalanceincidents.guesttechfilter");
            ...
            if (factionSource != null)
                ...
                Log.Warning("[HSKMoreBalanceEvents] GuestTechFilter: IncidentWorker_VisitorGroup.FactionCanBeGroupSource не найден — визиты от рассказчика не фильтруются.");
            ...
            if (DebugLog)
            {
                ...
                if (tryExecute != null)
                    ...
                    Log.Warning("[HSKMoreBalanceEvents] GuestTechFilter: IncidentWorker_VisitorGroup.TryExecuteWorker не найден — приходы гостей не логируются.");

                Log.Message($"[HSKMoreBalanceEvents] GuestTechFilter applied. DebugLog={DebugLog}, IiB active={IgnoranceCompat.Active}");
            }
        }
        ...
        public static bool PlanNewVisitPrefix(float afterDays, Faction faction)
        {
            ...
            if (DebugLog)
            {
                ...
                Log.Message($"[HSKMoreBalanceEvents] GuestTechFilter: план визита {DescribeFaction(faction)} через {afterDays:0.#} дн. -> {verdict}; " +
                    $"{DescribePlayerTech()}; вызвал {Caller()}");
            }
            else if (!run && Prefs.DevMode)
            {
                Log.Message($"[HSKMoreBalanceEvents] GuestTechFilter: визит {faction.Name} (тех {faction.def.techLevel}, игрок {IgnoranceCompat.PlayerTechLevel}) не запланирован.");
            }
            return run;
        }
        ...
        public static void FactionCanBeGroupSourcePostfix(Faction f, ref bool __result)
        {
            ...
            if (DebugLog && loggedCutFactions.Add(f.loadID))
                Log.Message($"[HSKMoreBalanceEvents] GuestTechFilter: визит от рассказчика — фракция {DescribeFaction(f)} ОТСЕЧЕНА; {DescribePlayerTech()}");
        }
        ...
        public static void VisitorArrivalPostfix(IncidentWorker __instance, IncidentParms parms, bool __result)
        {
            ...
            Log.Message($"[HSKMoreBalanceEvents] GuestTechFilter: ПРИХОД гостей ({__instance?.def?.defName}, результат {__result}) " +
        ...
        public static bool FillIncidentQueuePrefix(Map map)
        {
            ...
            if (DebugLog)
                Log.Message($"[HSKMoreBalanceEvents] GuestTechFilter: заполнение очереди, кандидаты: {string.Join(", ", candidates.Select(DescribeFaction))}; {DescribePlayerTech()}");
        ...
        private static string DescribePlayerTech()
        {
            return $"игрок по IiB {IgnoranceCompat.PlayerTechLevel} (IiB active={IgnoranceCompat.Active}), " +
                $"фракция игрока {Faction.OfPlayer?.def?.techLevel}, " +
                $"ahead={EventSettingsDef.GuestMaxTechAhead}, behind={EventSettingsDef.GuestMaxTechBehind}";
        }
